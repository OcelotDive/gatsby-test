...
const Ticker = () => {
  const tickerUrl =
    "https://financialmodelingprep.com/api/v3/quote/AAPL,FB,GOOGL,AMZN,MSFT,NVDA"
      ...
      return (
        <div className={tickerStyles.tickerStock} key={symbol.symbol}>
          <Link
            ...
            to={`/company-page/#${symbol.symbol}`}
          >
            <span className={tickerStyles.stockQuote}>
              {symbol.symbol} {symbol.price}
              <span className={tickerStyles[changeClass]}>
                {" "}
                {symbol.change} ({symbol.changesPercentage})
              </span>
            </span>
          </Link>
        </div>
